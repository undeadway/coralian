...
function equals(from, another) {
	...
	if (!typeIs(from, 'string')) Error.errorCast(from, String); // from 不是字符串
	if (!typeIs(another, 'string')) Error.errorCast(from, String); // another 不是字符串
...
if (!String.isEmpty) {
	String.isEmpty = function (input) {
		...
		if (!typeIs(input, 'string')) Error.unsupportedType(input);
